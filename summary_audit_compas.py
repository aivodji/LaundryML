...
def get_audit(data_prefix, exp_prefix, model_id):
    ...
    y_pred = y_pred[:,model_id].astype(int)

    for pos, val in enumerate(y_pred):    
            y_pred[pos] = 1 - y_pred[pos]

    clf.fit(dataset.values, y_pred)
...
scores["two_year_recid"] = scores["two_year_recid"].map({0 : 1, 1 : 0})
...
exp_prefix = "original/lambda=0.005__beta=0.9"
total_found = get_audit(data_prefix, exp_prefix, 26)

save_bbox = "plotting_scripts/graphs/audit_compas_bbox.eps"
save_badml = "plotting_scripts/graphs/audit_compas_badml.eps"
