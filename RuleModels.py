...
class CORELS_Enumerator(object):
    def __init__(self, rho=0.001, beta=0.0, opt='-b', branch_depth=3, minimal=True, k=10):
        self.rho_ = rho
        self.opt_ = opt
    ...
    def fit(self, data_file, itemset_file, itemset_name_file, minor_file):
        ...
        model = CORELS(rho=self.rho_, beta=self.beta_, opt=self.opt_, minimal=self.minimal_)
...
class CORELS(object):
    def __init__(self, rho=0.001, beta=0.0, opt='-b', minimal=True):
        self.rho_ = rho
        self.beta_ = beta
        self.opt_ = opt
        self.minimal_ = minimal
    ...
    def fit(self, data_file, label_file, itemset_file, itemset_name_file, minor_file, result_file):
        os.system('./algs/corels/src/corels -n 100000 -r %f -z %f %s %s %s %s > %s' % (self.rho_, self.beta_, self.opt_, data_file, label_file, minor_file, result_file))
